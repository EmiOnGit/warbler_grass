...
use bevy_inspector_egui::{prelude::*, inspector_options::std_options::NumberDisplay, bevy_egui::EguiContext, egui::{self, RichText, FontId}};

use super::{brush::{Stencil, Airbrush, BrushBehavior}, tools::{Eraser, Filler}, ray_cast::SelectedMap};
...
pub fn setup_ui(world: &mut World) {
    ...
    egui::Window::new("Editor").show(egui_context.get_mut(), |ui| {
            ui.label(RichText::new("Active Tool").font(FontId::proportional(20.0)).underline());

            bevy_inspector_egui::bevy_inspector::ui_for_resource::<ActiveTool>(world, ui);
            ui.add_space(section_distance);

            ui.push_id(1, |ui| {
                ui.label(RichText::new("Selected Map").font(FontId::proportional(20.0)).underline());
...
impl Brush {
    fn draw(&self, image: &mut Image, position: Vec2) {
        match &self.brush {
            BrushType::Stencil(stencil) => stencil.draw(image, position, self.brush_size, self.strength),
            BrushType::Airbrush(air_brush) => air_brush.draw(image, position, self.brush_size, self.strength),
        }
    }
}